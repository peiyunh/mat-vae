function [net,stats] = adam_train(net, imdb, getBatch, varargin)
...
if strcmp(mode,'train') && (isempty(net.m) || isempty(net.v))
  net.m = num2cell(zeros(1, numel(net.params))) ;
  net.v = num2cell(zeros(1, numel(net.params))) ;
end
...
if numGpus >= 1
  ...
  if strcmp(mode,'train')
    net.m = cellfun(@gpuArray,net.m,'UniformOutput',false) ;
    net.v = cellfun(@gpuArray,net.v,'UniformOutput',false) ;
  end
end
if numGpus > 1
...
for p=1:numel(net.params)
  ...
  switch net.params(p).trainMethod
    ...
    case 'gradient'
      net.m{p} = 0.9 * net.m{p} + 0.1 * net.params(p).der;
      net.v{p} = 0.999 * net.v{p} + 0.001 * net.params(p).der.^2;
      ...
      net.params(p).value = net.params(p).value ...
          - thisLR * thisDecay * net.params(p).value ...
          - thisLR * net.m{p}./(sqrt(net.v{p})+1e-8);
...
% -------------------------------------------------------------------------
sel = find(cellfun(@(x) isa(x,'dagnn.KLD')||isa(x,'dagnn.NLL')|| ...
                   isa(x,'dagnn.LB'), {net.layers.block})) ;
stats = struct() ;
for i = 1:numel(sel)
